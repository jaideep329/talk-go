...
- Keep using Daily app messages for frontend UI events and inbound control messages. Do not reintroduce a custom `/ws` route.
...
| File | Purpose |
|------|---------|
| root `main.go` | Entry point. Loads `.env`, configures logging (both terminal + `app.log`), initializes Disha deps, serves HTTP (`:3000`). Two HTTP routes only: `/connect` creates a `voicepipelinecore.PipelineTask` and joins the provided Daily room; `/` serves `daily-client.html`. `/connect` requires `room_url` and accepts `token`/`bot_token`; `conversation_id` requests route through `disha.NewBotTask` using `bot_type` (default `sales_call`). There is **no** custom WebSocket route — UI events ride Daily app messages (see `ui_events.go`) |
...
| `ui_events.go` | `UIEventType` enum, `UIEvent` struct (`Type` + `Data map[string]interface{}`), `UIEventSender` — publishes UI events as Daily app messages through `DailyRoom.SendAppMessage`. The room is plumbed in via `SetRoom` after `JoinDailyRoom` returns; pre-join `Send` calls silently no-op |
...
| root `daily-client.html` | Browser client: Pico CSS + Alpine.js. Dark theme. Posts to Disha backend `/bot/create_room` with a hardcoded local test user for sales-call testing, then joins the returned Daily room with `@daily-co/daily-js`. Live transcript, committed turns, multiple metrics badges per turn, connect/disconnect/mute controls |
...
**Generic UIEvents over Daily app messages:** `UIEvent` has typed `Type` (enum) + generic `Data map[string]interface{}`. Any processor can send any payload without struct changes. `UIEventSender.Send` publishes through `DailyRoom.SendAppMessage` via the Python bridge — no custom WebSocket route to host. The room is wired into the sender via `SetRoom` after `JoinDailyRoom` returns (`UIEventSender` was created earlier in `NewTask`). Browser → bot control messages (today `{"type":"end_call"}`) flow through Daily app messages and are handled in `daily_room.go`.
...
- **Transport:** Audio + UI events both ride Daily. There is **no** WebSocket to the talk-go service.
  - Outbound UI events: `_call.on("app-message", …)` dispatches by `event.type`.
