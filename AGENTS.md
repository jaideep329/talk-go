...
- Conversation turn observer workers must have explicit stop-and-drain before `PipelineTask` waits on the shared `WaitGroup`; do not rely only on task context cancellation or cleanup can time out before chunk sync is enqueued.
...
The pipeline is **complete and working end-to-end** with the Pipecat-style `BaseProcessor` design (single shared `WaitGroup`, two-channel system-frame priority, cancel-and-recreate on interrupt, explicit per-frame direction, frame-level auto-ID + name), barge-in (min-word detection with both in-progress and at-bot-turn-end back-channel discard), word timestamp tracking, multi-session support, Soniox/Cartesia websocket reconnection, live UI over the LiveKit data channel (no custom WebSocket), typed source-driven `EndTask` + `EndFrame` shutdown, `ErrorFrame` propagation, call events, conversation turn observer workers for committed turns, call stats tracking, user idle detection, talk-time enforcement, metrics framework, and a context aggregator that separates conversation orchestration from LLM execution. Each `/connect` request creates an independent `PipelineTask`.
...
| `metrics.go` | `MetricLabel` constants, `MetricsData` struct, `MetricsFrame` (intercepted by `BaseProcessor.PushFrame` → `taskCtx.Metrics`), `ProcessorMetrics` helper with `Start`/`StartAt`/`Stop`/`Reset` (thread-safe timer map), and `perTurnMetrics` snapshot/reset for turn observer commits |
| `call_events.go` | Once-only call event dispatcher (`OnBotJoined`, `OnUserJoined`, `OnFirstUserAudio`, `OnUserFirstSpeech`, `OnBotFirstSpeech`) that runs callbacks in goroutines tracked by the task `WaitGroup` and recovers panics |
| `conversation_turn_observer.go` | Per-task conversation turn observer worker set. Each `ConversationTurnObserver` gets a FIFO queue (cap 256); `runCleanup` calls `stopAndDrain()` before final `wg.Wait` so queued committed-turn writes finish before final call-ended work |
...
| `context_aggregator_test.go` | Final transcript → `LLMMessagesFrame`, initial-message seeding, explicit-empty initial context, once-only user-first-speech call event, conversation-turn observer emission with metric snapshot, barge-in emits InterruptFrame at min-words threshold, no barge-in below threshold, TTSDoneFrame commits assistant message, back-channel speech discarded both in-progress and at bot-turn-end (race-case fix), barge-in preserves accumulated transcript |
...
`TaskContext` is the single task-level dependency containing `Ctx` (root context for the task), `Logger`, `Room`, `UIEvents`, `Metrics` (handler), typed `EndTask`, and `wg` (shared WaitGroup for goroutine tracking), plus unexported call-events/call-stats/turn-observer/turn-metrics helpers.
...
**Single shared WaitGroup per PipelineTask:** `PipelineTask` owns `wg sync.WaitGroup`. `TaskContext.wg` is `&task.wg`. Every goroutine spawned via `BaseProcessor.Go`, call event dispatch, and conversation turn observer worker startup is tracked there. `PipelineTask.runCleanup` does `pipeline.Stop()`, disconnects the LiveKit room, stop-and-drains turn observers, then `wg.Wait()` with a bounded 10s timeout. No per-processor WaitGroup, no central TaskManager.
...
**Call events stay outside frames:** `TaskOptions` exposes once-only lifecycle hooks for integration code: bot joined, user joined, first audible user audio, first final user speech, first bot speech, and call ended. The first five are dispatched in goroutines tracked by the task `WaitGroup` and recover panics; `OnCallEnded` runs during `runCleanup` after turn observer drain and `wg.Wait`, with `EndReason` plus `CallStats`.

**Conversation turn observers are committed-turn hooks:** `ConversationTurnObserver` is not a frame observer. It receives `OnUserTurnCommitted(text, at)` from `ContextAggregator.addUserMessage` and `OnAssistantTurnCommitted(text, at, metrics)` from `commitSpokenText`. Each observer has one FIFO worker queue (cap 256); dispatch is non-blocking and drops with a log if the queue is full. `runCleanup` calls `stopAndDrain()` before `OnCallEnded`, so queued turn persistence completes before final sync/enqueue work.
