...
- **Audio formats**: Soniox expects s16le/16kHz/mono. Daily inbound audio is 16kHz mono PCM from the bridge; LiveKit inbound audio is Opus decoded/resampled by `PCMRemoteTrack` to 16kHz mono PCM for Soniox. Daily outbound bot audio stays 24kHz mono PCM because the Python bridge `CustomAudioSource` is 24k. LiveKit outbound requests Cartesia s16le/48kHz/mono, emits 20ms 48k PCM `AudioFrame`s, and creates `PCMLocalTrack` at 48k so the SDK's outbound path avoids a 24k→48k resample before Opus encoding. LiveKit still encodes to Opus because WebRTC audio tracks are Opus; there is no raw-PCM audio-track switch in the SDK helper. PlaybackSink derives frame size from the active room's output sample rate and resamples the background MP3 once at load time, not in the 20ms hot path. Do not reintroduce an internal Opus encode/decode round trip unless the output boundary itself changes to require encoded media.
...
- In diagnostics mode, use `audio_timing` log lines alongside Pyroscope when chasing jitter. They summarize 10s windows for 20ms-audio-path work such as Go/Python base64+JSON bridge I/O, LiveKit data publish/unmarshal, LiveKit PCM sample copy/conversion, playback tick lag/work, PCM frame copy/conversion, TTS JSON/base64 decode, and user PCM scanning. Profiles show CPU stacks; `audio_timing` shows deadline misses and where the real-time path is spending wall time. LiveKit SDK-internal Opus encode/decode loops are not exposed as per-frame timing hooks, so use Go Pyroscope plus `process_usage` for that CPU.
