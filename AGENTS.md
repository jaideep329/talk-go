...
- **LLM call logging parity** reuses Disha's existing `llm_logging_service` singleton: the router exposes an optional `LogSink` callback; `disha`'s `newLLMLogSink` fills it by enqueuing `services.llm_logging_service` / `llm_logging_service.log_llm_call_async` (S3 upload + DB save happen server-side) through the existing `/common/enqueue_job`. The sink is **not bot-specific** — the bot passes a `usecaseType` (e.g. `sales_call_conversation`) to differentiate call types. It is best-effort and size-guarded (skipped if kwargs exceed ~200KB to respect the SQS limit); token counts are not captured.
- **`voicepipelinecore` stays Redis-free.** The core defines an `LLMClient` interface (`Stream(ctx, messages, onToken) (LLMResult, error)`) on `LLMProcessor`; `NewLLMProcessor` keeps the default OpenAI gpt-4.1 client for the local `/connect` demo, and `NewLLMProcessorWithClient` injects the router. `llmrouter` is a pipeline sub-package that MAY import `net/http`/`crypto` and depends on a narrow `RedisStore` interface (satisfied structurally by `disha.RedisClient`); the core package never imports `llmrouter`. The `llm_call_result` RTVI event is emitted by `LLMProcessor` (with the real selected model), not by the pipeline metrics handler.
...
```
Browser Mic → Daily Room → Daily Python Bridge → Go Bot PCM → Soniox STT → LLM (sales: Grok via llmrouter; demo: OpenAI) → Cartesia TTS → Go PCM → Daily Python Bridge → Daily Room → Browser Speaker
```
...
The pipeline is **complete and working end-to-end** with the Pipecat-style `BaseProcessor` design (single shared `WaitGroup`, two-channel system-frame priority, cancel-and-recreate on interrupt, explicit per-frame direction, frame-level auto-ID + name), barge-in (min-word detection with both in-progress and at-bot-turn-end back-channel discard), word timestamp tracking, multi-session support, Soniox/Cartesia websocket reconnection, live UI over Daily app messages (no custom WebSocket), typed source-driven `EndTask` + `EndFrame` shutdown, `ErrorFrame` propagation, FIFO call events for lifecycle and committed turns, call stats tracking, user idle detection (with a 120s no-activity watchdog), talk-time enforcement, greet-first (bot speaks first on user-join), health-based LLM model switching (`llmrouter`), metrics framework, and a context aggregator that separates conversation orchestration from LLM execution. Each `/connect` request creates an independent `PipelineTask`.
...
| `disha/llm_logging.go` | `newLLMLogSink(api, logger, usecaseType, userID, conversationID)` — the router's `LogSink`. Enqueues `services.llm_logging_service`/`llm_logging_service.log_llm_call_async` via `/common/enqueue_job` (S3 + DB server-side). Size-guarded, best-effort, usecase-tagged (not bot-specific) |
...
| `llm_processor.go` | Lean: receives `LLMMessagesFrame` → spawns a Go-tracked `runLLM` goroutine that delegates the streaming HTTP call to an injected `LLMClient` (default = OpenAI gpt-4.1 via `llmEndpoint`; sales = `llmrouter`). The processor still owns `LLMResponseStart/End` frames, TTFB/processing metrics, cancel-on-Interrupt/End, and now emits the `llm_call_result` RTVI event with the client's real model. `NewLLMProcessorWithClient` injects a custom client; `NewLLMProcessor` keeps the default. `llmEndpoint` is a package var so tests can stub OpenAI via `httptest.NewServer` |
