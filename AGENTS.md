...
- Keep Disha sales-call setup shaped like Python `bots/sales_call/sales_call.py`, but split responsibilities in Go: `sales_call.go` is the high-level setup, `sales_call_startup.go` collects initial data, `call_operations.go` defines the bot-type-agnostic callback interface, `sales_call_operations.go` implements it for sales calls, `sales_call_events.go` registers callbacks, and `sales_call_end.go` owns end-call operations.
...
| `disha/session.go` | Generic Disha entrypoints and shared `Deps`; `BuildOptions`/`BuildSession` delegate to the sales-call implementation so the root binary can keep a stable entrypoint |
| `disha/call_operations.go` | Bot-type-agnostic `CallOperations` interface plus adapter to `voicepipelinecore.CallEvents`; future Disha bot types should implement this instead of adding core hooks |
| `disha/sales_call.go` | Main sales-call setup, shaped like Disha's Python `sales_call.py`: collect startup data, create sales-call operations, register callbacks, and return `voicepipelinecore.TaskOptions`; helper details stay out of this file |
| `disha/sales_call_operations.go` | Sales-call implementation of `CallOperations`: update-conversation lifecycle callbacks, committed-turn Redis chunk writes, and end-call delegation |
| `disha/sales_call_startup.go` | Sales-call startup data collection: load `conversation_data`, validate `sales_call`, resolve user ID, build initial messages, seed `hello?`, and apply remaining talk time |
| `disha/sales_call_events.go` | Mid-call callback registration for Disha update-conversation events and call-ended delegation |
| `disha/sales_call_end.go` | End-call operations: post-call request, end-reason mapping, and `/common/enqueue_job` chunk-sync trigger |
