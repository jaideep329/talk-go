...
| root `livekit-client.html` | Browser client: Pico CSS + Alpine.js. Dark theme. Posts to Disha backend `/bot/create_room` with a hardcoded local test user for sales-call testing, then joins the returned LiveKit room. Live transcript, committed turns, multiple metrics badges per turn, connect/disconnect/mute controls |
...
- **Connect path:** The Connect button posts `{user_id, resume_enabled:true}` to `http://localhost:8000/bot/create_room` using the hardcoded local test user ID in `livekit-client.html`, then joins the returned LiveKit `server_url`/`token`. The old `GET /connect` path still exists for direct talk-go tests, but the browser page exercises Disha's Create Room flow.
